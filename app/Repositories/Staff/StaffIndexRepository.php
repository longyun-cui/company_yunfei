...
//        $item_query->where(['item_status'=>1,'active'=>1]);
        ...
        else if($task_list_type == 'all')
        {
            ...
            $menu_active = 'sidebar_menu_task_list_for_all_active';
        }
        else if($task_list_type == 'unfinished')
        {
            ...
            $menu_active = 'sidebar_menu_task_list_for_unfinished_active';
        }
        else if($task_list_type == 'unpublished')
        {
            $item_query->whereIn('item_active',[0]);
            if($me->user_type == 11 || $me->user_type == 19)
            {
                $item_query->where('creator_id',$me->id);
            }
            $return['head_title'] = "待发布任务";
            $menu_active = 'sidebar_menu_task_list_for_unpublished_active';
        }
        else
        {
        }
...
//        $item_query->where(['item_status'=>1,'active'=>1]);
    ...
    {
        $me = Auth::guard('staff')->user();
        $view_template = env('TEMPLATE_STAFF_FRONT');
...
//        $return['item_type'] = $item_type;
        ...
        else
        {
            ...
            if($mine)
            {
//                if(!in_array($mine->user_type,[11,88])) return view(env('TEMPLATE_STAFF_FRONT').'errors.404');
...
//            ->where('active',1)
        ...
        $return['title_text'] = $title_text;
        return view(env('TEMPLATE_STAFF_FRONT').'entrance.user.staff-list')->with($return);
    }
    // 【用户】【全部】返回-列表-数据
